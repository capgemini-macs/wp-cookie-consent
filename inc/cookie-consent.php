...
add_action(
	...
	function () {
		...
		wp_enqueue_script( 'cookie-consent-js', esc_url( plugin_dir_url( __FILE__ ) ) . '../assets/js/cg-cookie-consent.js', [ 'lodash', 'jquery' ], '1.2.0' );
		wp_add_inline_script( 'cookie-consent-js', 'window.onload = window.runCookiesPlugin', 'after' );
	...
	function() {
		...
		if ( ! empty( $network_container_id ) ) {
			...
			<script type='text/javascript' data-name='cookie_network'>
				(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','<?php echo esc_js( $network_container_id ); ?>');
			</script>
			<?php
		}
